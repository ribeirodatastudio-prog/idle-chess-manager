...
const ParentNode = ({ skill, owned, canAfford, onPurchase }) => {
    return (
        <button
            onClick={() => !owned && canAfford && onPurchase(skill.id)}
            disabled={owned || !canAfford}
            className={`w-40 p-3 rounded-xl border transition-all flex flex-col items-center gap-2
                ${owned
            ...
            `}
        >
            <div className="font-bold text-sm text-center">{skill.name}</div>
            {owned ? (
...
export const SkillTreeModal = ({ isOpen, onClose, skills, derivedStats, onPurchase }) => {
    ...
    // Determine Lock State (Exclusivity)
    // If I own 'study_midgame', then Opening and Endgame are Locked.
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-[#1E1E24] w-full max-w-4xl rounded-2xl border border-gray-700 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-b border-gray-700 flex justify-between items-center bg-[#252529]">
                    <div>
                        ...
                        <p className="text-sm text-gray-400 mt-1">
                            Choose a Study Path to unlock specialized mastery trees. Paths are mutually exclusive.
                        </p>
                    </div>
                ...
                {/* Content */}
                <div className="p-8 overflow-y-auto flex-1 flex justify-center gap-8 md:gap-16">
